...
class Mainpage:
    ...
    def show_exercise_log(self):
        ...
        # Add new log button
        add_frame = tk.Frame(self.scrollable_frame, bg=self.colors['light'])
        add_frame.pack(pady=(0, 20))
        ...
        add_btn = tk.Button(add_frame,
                            ...
                            padx=20,
                            pady=10,
                            command=self.add_exercise_log)
        add_btn.pack()
        
        # Log entries
        if not self.exercise_log:
            empty_label = tk.Label(self.scrollable_frame,
                                    text="No exercise logs yet. Add your first workout!",
                                    font=('Arial', 14),
                                    fg=self.colors['text_light'],
                                    bg=self.colors['light'])
            empty_label.pack(pady=50)
        else:
            # Display logs in reverse (most recent first)
            for index, log in enumerate(reversed(self.exercise_log)):
                # index here is the display index (0 for most recent)
                self.create_log_entry(index, log)
    
    def create_log_entry(self, display_index, log):
        """Create a single log entry with delete button"""
        entry_frame = tk.Frame(self.scrollable_frame, bg=self.colors['card'])
        entry_frame.pack(fill='x', padx=30, pady=10)
        
        # Top row with date and delete button
        top_frame = tk.Frame(entry_frame, bg=self.colors['card'])
        top_frame.pack(fill='x', padx=20, pady=(10, 5))
        
        # Date
        date_label = tk.Label(top_frame,
                              text=log.get('date', 'Unknown Date'),
                              font=('Arial', 12, 'bold'),
                              fg=self.colors['primary'],
                              bg=self.colors['card'])
        ...
        # Delete button
        delete_btn = tk.Button(top_frame,
                              text="🗑️ Delete",
                              font=('Arial', 10),
                              ...
                              padx=10,
                              pady=3,
                              command=lambda idx=display_index: self.delete_log_entry(idx))
        delete_btn.pack(side='right')
        
        # Exercise name
        exercise_label = tk.Label(entry_frame,
                                  text=f"Exercise: {log.get('exercise', 'Unknown')}",
                                  font=('Arial', 11),
                                  fg=self.colors['text'],
                                  bg=self.colors['card'])
        ...
        # Details
        details_frame = tk.Frame(entry_frame, bg=self.colors['card'])
        details_frame.pack(fill='x', padx=20, pady=(0, 10))
        
        details = [
            (f"Duration: {log.get('duration', 'N/A')} mins"),
            (f"Calories: {log.get('calories', 0)}"),
            (f"Notes: {log.get('notes', 'No notes')}")
        ]
        ...
        for detail in details:
            detail_label = tk.Label(details_frame,
                                     ...
                                     fg=self.colors['text_light'],
                                     bg=self.colors['card'])
            detail_label.pack(side='left', padx=(0, 20))

    def delete_log_entry(self, display_index):
        """Delete a log entry after confirmation"""
        # Since we display in reverse order, we need to find the actual index
        actual_index = len(self.exercise_log) - 1 - display_index
        
        if 0 <= actual_index < len(self.exercise_log):
            log_to_delete = self.exercise_log[actual_index]
            
            # Ask for confirmation
            response = messagebox.askyesno(
                "Delete Exercise Log",
                f"Are you sure you want to delete this exercise log?\n\n"
                f"Exercise: {log_to_delete.get('exercise', 'Unknown')}\n"
                f"Date: {log_to_delete.get('date', 'Unknown')}\n\n"
                f"This action cannot be undone."
            )
            
            if response:
                # Delete the log entry
                self.exercise_log.pop(actual_index)
                self.save_data()
                
                # Refresh the exercise log screen
                self.show_exercise_log()
                
                messagebox.showinfo("Success", "Exercise log deleted successfully!")
        else:
            messagebox.showerror("Error", "Could not find the exercise log to delete.")
    
    def add_exercise_log(self):
        """Add new exercise log dialog"""
        dialog = tk.Toplevel(self.root)
        dialog.title("Add Exercise Log")
        dialog.geometry("500x400")
        dialog.configure(bg=self.colors['light'])
        
        # Form fields
        fields = [
            ("Exercise:", tk.Entry(dialog, font=('Arial', 11), width=30)),
            ("Duration (mins):", tk.Entry(dialog, font=('Arial', 11), width=30)),
            ("Calories Burned:", tk.Entry(dialog, font=('Arial', 11), width=30)),
            ("Notes:", tk.Text(dialog, font=('Arial', 11), height=4, width=30))
        ]
        ...
        for i, (label_text, widget) in enumerate(fields):
            label = tk.Label(dialog,
                             ...
                             fg=self.colors['text'],
                             bg=self.colors['light'])
            label.pack(pady=(20 if i == 0 else 10, 5))
            widget.pack()
        
        # Pre-fill calories/duration with a placeholder for better UX
        fields[1][1].insert(0, '30')
        fields[2][1].insert(0, '200')


        def save_log():
            exercise = fields[0][1].get().strip()
            duration_str = fields[1][1].get().strip()
            calories_str = fields[2][1].get().strip()
            notes = fields[3][1].get("1.0", tk.END).strip()
            
            # Validation
            if not exercise:
                messagebox.showwarning("Missing Information", "Please enter the exercise name.")
                return
            
            if not duration_str.isdigit() or int(duration_str) <= 0:
                messagebox.showwarning("Invalid Input", "Duration must be a positive whole number.")
                return

            try:
                calories = int(calories_str) if calories_str.isdigit() and int(calories_str) >= 0 else 0
            except ValueError:
                messagebox.showwarning("Invalid Input", "Calories must be a non-negative whole number.")
                return
            ...
            self.exercise_log.append(log_entry)
            self.save_data()
            dialog.destroy()
            self.show_exercise_log()
    ...
    def show_calorie_calculator(self):
        ...
        # Calculator frame
        calc_frame = tk.Frame(self.scrollable_frame, bg=self.colors['card'])
        calc_frame.pack(fill='x', padx=50, pady=20)
        
        # Exercise selection
        exercise_label = tk.Label(calc_frame,
                                  text="Select Exercise:",
                                  font=('Arial', 12, 'bold'),
                                  fg=self.colors['text'],
                                  bg=self.colors['card'])
        exercise_label.pack(pady=(20, 10))
        
        exercise_var = tk.StringVar()
        exercise_combo = ttk.Combobox(calc_frame,
                                       textvariable=exercise_var,
                                       values=[p['name'] for p in self.programs],
                                       font=('Arial', 11),
                                       width=40)
        exercise_combo.pack(pady=(0, 20))
        exercise_combo.set("Select an exercise")
        
        # Duration input
        duration_frame = tk.Frame(calc_frame, bg=self.colors['card'])
        ...
        # Weight input (auto-filled from profile)
        weight_frame = tk.Frame(calc_frame, bg=self.colors['card'])
        ...
        # Auto-fill from profile, defaulting to '70' if N/A
        profile_weight = self.user_profile.get('Weight', '70')
        weight_entry.insert(0, str(profile_weight) if str(profile_weight).replace('.', '').isdigit() else '70')
        ...
        def calculate_calories():
            ...
            if not exercise or exercise == "Select an exercise":
                result_label.config(text="Please select an exercise")
                return
            
            if not duration.isdigit() or int(duration) <= 0:
                result_label.config(text="Please enter valid positive duration (minutes)")
                return
            
            try:
                weight_float = float(weight)
                if weight_float <= 0: raise ValueError
            except ValueError:
                result_label.config(text="Please enter valid positive weight (kg)")
                return
            
            # Simple calorie calculation (MET values * duration * weight)
            met_values = {
                'Push-up Program': 3.8,
                'Sit-up Challenge': 3.0,
                'Squat Mastery': 5.0,
                'Plank Progression': 2.5,
                'Pull-up Training': 4.0,
                'Lunge Program': 4.5
            }
            
            met = met_values.get(exercise, 3.5) # Default MET value for general exercise
            duration_int = int(duration)
            
            # Calories = MET * weight(kg) * time(hours)
            calories = met * weight_float * (duration_int / 60)
            
            result_label.config(text=f"Estimated Calories Burned: {calories:.1f} calories")
    ...
    def show_workout_schedule(self):
        """Show workout schedule screen"""
        self.destroy_quick_stats()
        self.clear_content()
        ...
        # Schedule new workout button
        schedule_btn = tk.Button(self.scrollable_frame,
                                 text="➕ Schedule New Workout",
                                 font=('Arial', 12, 'bold'),
                                 bg=self.colors['success'],
                                 fg='white',
                                 padx=20,
        ...
        schedule_btn.pack(pady=(0, 30))
        
        # Display schedule
        if not self.schedule:
            empty_label = tk.Label(self.scrollable_frame,
                                    text="No workouts scheduled. Schedule your first workout!",
                                    font=('Arial', 14),
                                    fg=self.colors['text_light'],
                                    bg=self.colors['light'])
            empty_label.pack(pady=50)
            ...
            for date_str in sorted_dates:
                schedule_frame = tk.Frame(self.scrollable_frame, bg=self.colors['card'])
                schedule_frame.pack(fill='x', padx=30, pady=10)
                
                date_label = tk.Label(schedule_frame,
                                      text=date_str,
                                      font=('Arial', 12, 'bold'),
                                      fg=self.colors['primary'],
                                      bg=self.colors['card'])
                ...
                workout_label = tk.Label(schedule_frame,
                                         text=f"Workout: {self.schedule[date_str]}",
                                         font=('Arial', 11),
                                         fg=self.colors['text'],
                                         bg=self.colors['card'])
                workout_label.pack(anchor='w', padx=20, pady=(0, 10))
    
    def schedule_workout(self):
        """Schedule a new workout dialog"""
        dialog = tk.Toplevel(self.root)
        dialog.title("Schedule Workout")
        dialog.geometry("500x300")
        dialog.configure(bg=self.colors['light'])
        
        # Date selection
        date_label = tk.Label(dialog,
        ...
        date_entry = tk.Entry(dialog,
                              ...
                              width=20)
        date_entry.pack()
        # timedelta is now safely imported at the top
        date_entry.insert(0, (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")) # Default to tomorrow
        
        # Workout selection
        workout_label = tk.Label(dialog,
                                 text="Select Workout:",
                                 font=('Arial', 12, 'bold'),
                                 fg=self.colors['text'],
                                 bg=self.colors['light'])
        workout_label.pack(pady=(20, 10))
        
        workout_var = tk.StringVar()
        workout_combo = ttk.Combobox(dialog,
                                      textvariable=workout_var,
                                      values=[p['name'] for p in self.programs],
                                      font=('Arial', 11),
                                      width=30)
        workout_combo.pack()
        workout_combo.set("Select a workout")
        
        def save_schedule():
            date_str = date_entry.get().strip()
            workout = workout_var.get()
            
            if not date_str or workout == "Select a workout":
            ...
            self.schedule[date_str] = workout
            self.save_data()
            dialog.destroy()
            self.show_workout_schedule()
